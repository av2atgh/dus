...
\item The small-world claim has been withdrawn in its strong form, as
Reviewer 1 requested, and replaced by the precise statement the data support.
...
multiplicative errors, $1.56\,x^{0.69}$, ahead of the linear relation by
$\Delta\mathrm{AIC} = 314$ and the two-parameter logarithm by
$\Delta\mathrm{AIC} = 1328$. The exponent is $0.694$ with a 95\% bootstrap
confidence interval of $[0.679, 0.709]$, comfortably excluding $1$, and
...
($p = 5 \times 10^{-9}$).
\item DPS: a shared exponent is not rejected ($\mathrm{LR} = 1.4$,
$\mathrm{df} = 2$, $p = 0.50$; shared value $0.71$), while the prefactors differ
($p = 4 \times 10^{-7}$).
\end{itemize}
...
\reply Accepted, and the claim has been withdrawn in its strong form. The
reviewer is right on both counts: falling below the chain expectation is nearly
tautological, and the Watts--Strogatz definition requires a clustering
comparison that the paper did not make. I would add that the difficulty is more
than one of effort --- the standard clustering coefficient counts closed
triangles, and in a DAG a directed triangle cannot close, so the quantity is not
merely unmeasured but undefined without a new construction. Choosing a
DAG-appropriate clustering statistic and its correct degree-preserving null
model is a research question in its own right, and I did not want to invent one
in order to rescue a label.

The paper now makes the weaker claim the data actually support, in the terms the
reviewer suggested. The relevant text reads that shortcuts make the DUS network
shallow and that its depth does not grow appreciably with project size. The
supporting evidence has been strengthened beyond the chain comparison, because
the interesting fact turned out not to be the gap but the flatness:
...
per level, 95\% CI $[0.0019, 0.0062]$). The implied growth is negligible: across
...
attributed to Watts and Strogatz, not as a claimed property of the networks. The
Discussion lists the definition of clustering in a DAG as an open question.
...
a median observed/predicted ratio of $0.76$. The power law fitted within PSPLIB
